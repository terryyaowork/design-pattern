...
│   │   ├── flyweight/                      # 享元模式
│   │   │   ├── TransactionType.js          # 交易類型類別，用於表示不同的交易類型
│   │   │   ├── TransactionFactory.js       # 交易類型工廠，管理交易類型的創建與重用
│   │   │   ├── TransactionRecord.js        # 交易紀錄類別，包含交易詳細信息
│   │   ├── prototype/                      # 原型模式
...
Flyweight 模式是一種結構型設計模式，它旨在通過共享儲存來減少大量物件的內存使用。這種模式特別適用於大量相似物件的場景，通過將它們的共用部分抽取出來共享使用，從而節省內存。

#### 使用情境

1. **交易紀錄系統**：
    - Flyweight 模式可以用來管理大量交易紀錄，通過重用相同的交易類型實例來節省內存空間。
2. **字型渲染系統**：
    - 在處理大量文字顯示時，可以使用 Flyweight 模式來重用相同的字型對象，減少內存開銷。
3. **遊戲開發**：
    - 在遊戲中用於重用相同類型的圖像資源，例如樹木、建築物等，避免每個物件都創建獨立的實例。
